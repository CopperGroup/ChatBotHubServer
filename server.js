...
app.get("/widget/chatbot-widget.js", async (req, res) => {
  ...
  try {
    ...
    if (!website) {
      console.log(
        `Chatbot widget request: Website with chatbotCode ${chatbotCode} not found.`
      );
    ...
    if (!shouldDisplayWidget) {
      console.log(
        `[Chatbot Widget] Not sending widget script for chatbotCode ${chatbotCode}. Reason: ${reasonForNotDisplaying}`
      );
      ...
      return res
        ...
        .send(
          `// Chatbot widget not loaded. Reason: ${reasonForNotDisplaying}`
        );
    }
    ...
    const fullScript = `window.chatbotConfig = ${JSON.stringify(
      injectedConfig
    )};\n${clientScriptContent}`;
