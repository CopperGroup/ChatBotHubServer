...
export async function handleWidgetMessage(socket, io, website, { chatId, email, message, currentWebsiteURL: clientUrlFromMessage }) {
    ...
    try {
        ...
        if (workflowData && chat.currentWorkflowBlockId) {
            ...
            const currentBlock = getBlockById(workflowData, chat.currentWorkflowBlockId);
            if (currentBlock && currentBlock.type === "end") {
                console.log(`SERVER FLOW: Current workflow block is an 'end' block (${currentBlock.id}). Quitting workflow processing for new user message.`);
                ...
                if (workflowAdvanceResult.responses.length > 0) {
                    ...
                    if (concatenatedMessage || finalOptions.length > 0) {
                        ...
                        console.log(`SERVER FLOW: Workflow generated bot message: "${workflowBotMessage.text}" with options: ${JSON.stringify(workflowBotMessage.options)}.`);

                        socket.emit("bot_typing_start");
                        console.log(`SERVER DEBUG: Emitted 'bot_typing_start' for workflow bot message.`);
                        ...
                        console.log(`SERVER DEBUG: Emitted 'reply' for workflow message. Text: "${workflowBotMessage.text}".`);
                        socket.emit("bot_typing_stop");
                        console.log(`SERVER DEBUG: Emitted 'bot_typing_stop' after workflow bot message.`);
                    } else {
                    ...
                    if (workflowPathEnded && nextWorkflowBlockToSave && nextWorkflowBlockToSave.includes("end")) {
                        ...
                        console.log("SERVER DEBUG: Telegram notification sent for workflow path completion.");
                    ...
                    if (workflowAdvanceResult.nextWorkflowBlockId && workflowAdvanceResult.nextWorkflowBlockId !== chat.currentWorkflowBlockId) {
                        nextWorkflowBlockToSave = workflowAdvanceResult.nextWorkflowBlockId;
                        console.log(`SERVER FLOW: Workflow advanced internally to ${nextWorkflowBlockToSave} but produced no user message this turn.`);
                    }
            ...
            console.log(`SERVER FLOW: No active workflow (${chat.currentWorkflowBlockId}) or workflow data not found. Skipping workflow processing, AI will handle.`);
        ...
        console.log(`    - Workflow produced message (this turn): ${workflowBotMessage !== null}`);
        ...
        if (shouldEngageAI) {
            ...
            if (TOKEN_SERVICE_BASE_URL) {
                ...
                try {
                    ...
                    if (currentDailyUsageResponse.ok) {
                        ...
                        console.log(`SERVER DEBUG: Current daily token usage for ${website._id} on ${todayDate}: ${todayTokenUsage}`);
                    } else {
            ...
            if (messages.length === 2 && chat.name === chat.email.split("@")[0] && !aiConditionsMet) {
                ...
                try {
                    ...
                    if (aiResponse.ok) {
                        ...
                        if (receivedAiResponseText && receivedAiResponseText.toLowerCase().includes("code:human007")) {
                            console.log("SERVER INFO: AI requested human assistance (code:human007). Overriding AI message.");
                            aiHandoverTelegramNotificationNeeded = true;
                            chat.aiResponsesEnabled = false;
                            await chat.save();
                            aiBotMessage = {
                                sender: "ai",
                                text: multiLanguage["Ok, I already transferred your message to the staff team, they will join this chat soon."][website.preferences.language || "en"],
                                timestamp: new Date().toISOString(),
                            };
                        } else {
                            aiBotMessage = {
                        ...
                        aiBotMessage = {
                            sender: "bot",
                            text: multiLanguage["Error processing your message with AI. Please try again or wait for staff assistance."][website.preferences.language || "en"],
                            timestamp: new Date().toISOString(),
                        };
                        messages.push(aiBotMessage);
                    }
                    ...
                    aiBotMessage = {
                        sender: "bot",
                        text: multiLanguage["There was a problem reaching our AI. Please try again later."][website.preferences.language || "en"],
                        timestamp: new Date().toISOString(),
                    };
                    messages.push(aiBotMessage);
                } finally {
                ...
                if (aiBotMessage) {
                    socket.emit("bot_typing_start");
                    console.log(`SERVER DEBUG: Emitted 'bot_typing_start' for AI bot message.`);
                    socket.emit("reply", {
                    ...
                    console.log(`SERVER DEBUG: Emitted 'reply' for AI message. Text: "${aiBotMessage.text}".`);
                    socket.emit("bot_typing_stop");
                    console.log(`SERVER DEBUG: Emitted 'bot_typing_stop' after AI bot message.`);
                }
            ...
            console.log("SERVER DEBUG: No bot message generated by workflow, and AI was not engaged. This turn will be silent to the user.");
            telegramNotificationForUnhandledMessage = true;
        }
        ...
        console.log(`SERVER DEBUG: Messages and workflow state saved to DB for chat ${chatId}. Final workflow position: ${nextWorkflowBlockToSave}. Chat status: ${chat.status}.`);
        ...
        if (aiHandoverTelegramNotificationNeeded) {
            ...
            console.log(`SERVER DEBUG: Telegram notification sent: AI requested human assistance.`);
        }
        // Trigger Telegram notification for messages received after workflow completion or when no workflow/AI response
            ...
            console.log(`SERVER DEBUG: Telegram notification sent: User message received after workflow concluded or without bot response.`);
        }

    } catch (error) {
        console.error(`SERVER ERROR: Error handling message for chat ${chatId}:`, error);
        socket.emit("reply", { text: "An unexpected error occurred. Please try again.", sender: "bot", timestamp: new Date().toISOString() });
        socket.emit("bot_typing_stop");
    }
}