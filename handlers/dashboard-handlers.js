 ...
 */
export async function handleDashboardMessage(socket, io, data, dashboardUser, isStaff, staffId) {
    ...
    try {
        ...
        console.log("STAFFF", chat.leadingStaff ? chat.leadingStaff.toString() : "No leading staff");
        if (website.owner) {
 ...
 */
export async function handleToggleAIResponses(io, { chatId, enable, userId, isStaff, staffName }) {
    console.log(`SERVER DEBUG: Received 'toggle_ai_responses' from ${isStaff ? 'Staff' : 'Dashboard'} (Socket ID: N/A - via handler). Chat ID: ${chatId}. Enable: ${enable}.`);
    try {
        const chat = await Chat.findById(chatId);
        if (!chat) {
 ...
 * @param {object} data - The event data.
 */
export async function handleCloseChat(io, { chatId, closerId, closerType, closerName, websiteId }) {
 ...
 * @param {object} data - The event data.
 */
export async function handleAssignChatLead(io, { chatId, assigneeId, assigneeName, assigneeType, websiteId }) {
 ...
 * @param {object} data - The event data.
 */
export async function handleUnassignChatLead(io, { chatId, assigneeId, assigneeType, websiteId }) {
